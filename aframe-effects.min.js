	...
	return __webpack_require__(0)
			...
			AFRAME.registerSystem("effects", {
				...
				renderPass: function (material, renderTarget, viewCb, forceClear) {
					...
					if (viewCb) {
						if (this.cameras.length > 1) {
							...
							if (isFn)
								viewCb(material, this.cameras[0], -1);
							...
							if (isFn)
								viewCb(material, this.cameras[1], 1);
							...
							setView(0, 0, s.width, s.height);
							if (isFn)
								viewCb(material, this.sceneEl.camera, 0);
				...
				fuse: function (temp, alpha) {
					...
					if (this.sceneEl.components.debug)
				...
				tick: function (time, timeDelta) {
					...
					this.passes.forEach(function (p) {
						if (p.behavior && p.behavior.bypass === true)
							return;
						arr.push(p)
					});
				...
				resizeTiles: function () {
					...
					LightState.tileData.value = [width, height, .5 / Math.ceil(width / 32), .5 / Math.ceil(height / 32)];
				...
				tileLights: function (renderer, scene, camera) {
					...
					var passes;
					if (camera instanceof THREE.ArrayCamera) {
						passes = [[.5, 0, camera.cameras[0]], [.5, .5, camera.cameras[1]]]
					} else {
						passes = [1, 0, camera]
					}
					passes.forEach(function (pass) {
						lights.forEach(function (light, index) {
							...
							if (bs[2] < 0)
								bs[2] = 0;
							if (bs[3] > LightState.height)
								bs[3] = LightState.height;
			...
			AFRAME.registerComponent("outline", {
				...
				tock: function () {
					if (!this.system.isActive(this, true))
						return;
			...
			AFRAME.registerComponent("bloom", {
				...
				tock: function (time) {
					if (!this.data.enable || !this.system.isActive(this, true))
						return;
